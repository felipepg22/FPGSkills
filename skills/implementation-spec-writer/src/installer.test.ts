...
test("installs the canonical skill into all native project targets", async () => {
  ...
  for (const destination of destinations) {
    const skill = await readFile(path.join(project, destination, "implementation-spec-writer", "SKILL.md"), "utf8");
    assert.match(skill, /name: implementation-spec-writer/);
  }
});

test("keeps successful targets when another target conflicts", async () => {
  const project = await temporaryProject();
  const conflict = path.join(project, ".cursor/skills/implementation-spec-writer/SKILL.md");
  ...
  await readFile(path.join(project, ".codex/skills/implementation-spec-writer/SKILL.md"), "utf8");
...
test("status detects modification and uninstall preserves modified files", async () => {
  ...
  const skillFile = path.join(project, ".codex/skills/implementation-spec-writer/SKILL.md");
...
async function temporaryProject(): Promise<string> {
  return mkdtemp(path.join(tmpdir(), "implementation-spec-writer-"));
}
