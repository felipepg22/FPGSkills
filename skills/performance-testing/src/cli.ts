...
const HELP = `Performance Testing installer

Usage:
  performance-testing
  performance-testing install --target <target> --scope <local|global> [options]
  performance-testing uninstall --scope <local|global> [--target <target>] [--force]
  performance-testing status --scope <local|global> [--json]
...
async function main(): Promise<void> {
  ...
  switch (args.command) {
    ...
    case "status": {
      ...
      else if (entries.length === 0) stdout.write("No Performance Testing files are recorded in this scope.\n");
...
function printResults(results: OperationResult[]): void {
  if (results.length === 0) {
    stdout.write("No matching Performance Testing files found.\n");
    return;
  }
