...
const HELP = `FPG AGENTS.md Writer installer

Usage:
  fpg-agents-md-writer
  fpg-agents-md-writer install --target <target> --scope <local|global> [options]
  fpg-agents-md-writer uninstall --scope <local|global> [--target <target>] [--force]
  fpg-agents-md-writer status --scope <local|global> [--json]
...
async function main(): Promise<void> {
  ...
  switch (args.command) {
    ...
    case "status": {
      ...
      else if (entries.length === 0) stdout.write("No FPG AGENTS.md Writer files are recorded in this scope.\n");
...
function printResults(results: OperationResult[]): void {
  if (results.length === 0) {
    stdout.write("No matching FPG AGENTS.md Writer files found.\n");
    return;
  }
